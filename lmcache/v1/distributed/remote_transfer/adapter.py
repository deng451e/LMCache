...
"""Abstract interface for remote memory transfer (data-plane transport)."""
...
class LocalMemHandle:
    ...
    Returned by RemoteTransferAdapter.register_local_memory(). Reused across
    all transfers against the same buffer — register once at startup.
    """
...
class RemoteMemHandle:
    ...
    Returned by RemoteTransferAdapter.connect_peer(). Stored in the
    RemoteController peer registry and passed to read/write at transfer time.
    """
...
class RemoteTransferAdapter(ABC):
    """Pure data-plane transport for remote memory transfers.

    No protocol knowledge, no peer state, no ZMQ. Called by the application
    layer after RemoteController.lookup() returns remote handles and page
    indices.
    """
